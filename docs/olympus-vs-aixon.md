...
| **3.2** | ✅ FIXED | Both adapters' `format_stream_chunk` now builds the delta/events additively, so a `Chunk(content=…, done=True)` or `Chunk(content=…, reasoning=…)` keeps every field. `test_adapter_stream_multifield.py` covers content+done and content+reasoning on both dialects. |
| **3.3** | ✅ FIXED | `max_execution_time`/`timeout` are now real **post-hoc deadlines**: `ToolAgent.invoke` raises `AixonError` when exceeded (was a silent log); `Orchestrator.invoke` already raised. Docstrings + `docs/orchestrator.md` corrected — they are deadlines (checked after the run / between stream steps), not interrupts of an in-flight tool call. `test_tool_agent_deadline.py`. |
| **3.5** | ✅ FIXED | `Registry.resolve` no longer returns the lone agent for an arbitrary name — the single-agent fallback now triggers only for an **empty/missing** model, so a typo'd model raises `AgentNotFoundError`. `test_registry.py` updated. |
| **3.6** | ✅ FIXED | `reset_registry()` now clears `Agent._registered` on every subclass, so a class re-registers into the fresh registry instead of short-circuiting on a stale flag. `test_registry_reset_reregisters.py`. |
| **3.7** | ✅ FIXED | `from_langchain` reads `tool_call_id`/`name` back, so a tool `Message` survives a `Message→LangChain→Message` round-trip. `test_interop_tool_roundtrip.py`. |
| **3.8** | ✅ FIXED | `aixon serve --anthropic` mounts both dialects (OpenAI at `/v1`, Anthropic at `/anthropic`); `Server(adapters=…)` now raises `AixonError` if it would silently ignore a different adapter set on the singleton. `test_cli_serve.py` + `test_server_multi_adapter.py`. |
| **3.9** | ✅ DOCUMENTED | `usage` is intentionally `{}` — the neutral `Message` carries no token counts. Noted in `docs/server.md` and a code comment; wiring real counts would require a boundary change (deferred). |
